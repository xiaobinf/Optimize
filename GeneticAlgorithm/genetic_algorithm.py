...
def fitness(population_size, chromosome_size):
    ...
    for i in range(0, population_size):
        ...
        fitness_value[i] = lower_bound + fitness_value[i] * \
                                         (upper_bound - lower_bound) / (pow(2, chromosome_size) - 1)  # 将十进制投影到0-9区间
        # fitness_value[i]=0 + fitness_value[i]*(9 - 0) / (pow(2,10) - 1)
        fitness_value[i] = fitness_value[i] + 10 * np.sin(
            5 * fitness_value[i]) + 7 * np.cos(4 * fitness_value[i])  # 计算自变量xi的适应度函数值
    ...
    for i in range(0, population_size, 2):
        if random.random() < cross_rate:
            ...
            for j in range(cross_position, chromosome_size):
                population[i][j], population[i + \
                                             1][j] = population[i + 1][j], population[i][j]
